...
This guide provides comprehensive instructions for setting up and using GitHub Projects for culture-architecture-system development.
...
- 👷 `user: manager` - Daily wage workers
- 🏗️ `user: executive` - Executives
- 📋 `user: hr` - NREGA workers
- 👥 `user: all` - All users
...
5. Name: `culture-architecture-system Development`
6. Click **Create**
...
```
✅ Good: bug, priority: high, area: auth, user: manager
❌ Bad: bug (too vague)
```
...
gh issue list --label bug --json number \
  | jq '.[].number' \
  | xargs -I {} gh project item-add [PROJECT_NUMBER] --owner akaash-nigam --url "https://github.com/akaash-nigam/visionOS_culture-architecture-system/issues/{}"
```
