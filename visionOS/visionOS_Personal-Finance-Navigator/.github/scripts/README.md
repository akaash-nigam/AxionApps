...
Automated scripts for configuring the Personal-Finance-Navigator GitHub repository.
...
**View labels:**
```bash
gh label list --repo akaash-nigam/visionOS_Personal-Finance-Navigator
```
...
**View milestones:**
```bash
gh api repos/akaash-nigam/visionOS_Personal-Finance-Navigator/milestones --jq '.[] | {title, due_on, open_issues}'
```
...
1. Go to: https://github.com/akaash-nigam/visionOS_Personal-Finance-Navigator/projects
2. Click **New project** → **Team backlog** template
3. Name: "Personal-Finance-Navigator Development"
...
```bash
gh label list --repo akaash-nigam/visionOS_Personal-Finance-Navigator --json name \
  | jq -r '.[].name' \
  | xargs -I {} gh label delete {} --repo akaash-nigam/visionOS_Personal-Finance-Navigator --yes
```

### Delete all milestones

```bash
gh api repos/akaash-nigam/visionOS_Personal-Finance-Navigator/milestones --jq '.[].number' \
  | xargs -I {} gh api -X DELETE repos/akaash-nigam/visionOS_Personal-Finance-Navigator/milestones/{}
```

### Export labels to JSON

```bash
gh label list --repo akaash-nigam/visionOS_Personal-Finance-Navigator --json name,color,description > labels.json
```
...
gh issue list --label bug --json number \
  ...
  | xargs -I {} gh project item-add $PROJECT_NUMBER \
    --owner akaash-nigam \
    --url "https://github.com/akaash-nigam/visionOS_Personal-Finance-Navigator/issues/{}"
```
