...
This guide provides comprehensive instructions for setting up and using GitHub Projects for Gaming spatial-arena-championship development.
...
- 👷 `user: gamer` - Daily wage workers
- 🏗️ `user: spectator` - Spectators
- 📋 `user: organizer` - NREGA workers
- 👥 `user: all` - All users
...
5. Name: `Gaming spatial-arena-championship Development`
6. Click **Create**
...
```
✅ Good: bug, priority: high, area: auth, user: gamer
❌ Bad: bug (too vague)
```
...
gh issue list --label bug --json number \
  | jq '.[].number' \
  | xargs -I {} gh project item-add [PROJECT_NUMBER] --owner akaash-nigam --url "https://github.com/akaash-nigam/visionOS_Gaming_spatial-arena-championship/issues/{}"
```
