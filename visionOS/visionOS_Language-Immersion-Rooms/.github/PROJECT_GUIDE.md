...
This guide provides comprehensive instructions for setting up and using GitHub Projects for Language-Immersion-Rooms development.
...
- 👷 `user: learner` - Daily wage workers
- 🏗️ `user: native speaker` - Native Speakers
- 📋 `user: teacher` - NREGA workers
- 👥 `user: all` - All users
...
5. Name: `Language-Immersion-Rooms Development`
6. Click **Create**
...
```
✅ Good: bug, priority: high, area: auth, user: learner
❌ Bad: bug (too vague)
```
...
gh issue list --label bug --json number \
  | jq '.[].number' \
  | xargs -I {} gh project item-add [PROJECT_NUMBER] --owner akaash-nigam --url "https://github.com/akaash-nigam/visionOS_Language-Immersion-Rooms/issues/{}"
```
