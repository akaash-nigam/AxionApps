...
Automated scripts for configuring the Language-Immersion-Rooms GitHub repository.
...
**View labels:**
```bash
gh label list --repo akaash-nigam/visionOS_Language-Immersion-Rooms
```
...
**View milestones:**
```bash
gh api repos/akaash-nigam/visionOS_Language-Immersion-Rooms/milestones --jq '.[] | {title, due_on, open_issues}'
```
...
1. Go to: https://github.com/akaash-nigam/visionOS_Language-Immersion-Rooms/projects
2. Click **New project** → **Team backlog** template
3. Name: "Language-Immersion-Rooms Development"
...
```bash
gh label list --repo akaash-nigam/visionOS_Language-Immersion-Rooms --json name \
  | jq -r '.[].name' \
  | xargs -I {} gh label delete {} --repo akaash-nigam/visionOS_Language-Immersion-Rooms --yes
```

### Delete all milestones

```bash
gh api repos/akaash-nigam/visionOS_Language-Immersion-Rooms/milestones --jq '.[].number' \
  | xargs -I {} gh api -X DELETE repos/akaash-nigam/visionOS_Language-Immersion-Rooms/milestones/{}
```

### Export labels to JSON

```bash
gh label list --repo akaash-nigam/visionOS_Language-Immersion-Rooms --json name,color,description > labels.json
```
...
gh issue list --label bug --json number \
  ...
  | xargs -I {} gh project item-add $PROJECT_NUMBER \
    --owner akaash-nigam \
    --url "https://github.com/akaash-nigam/visionOS_Language-Immersion-Rooms/issues/{}"
```
